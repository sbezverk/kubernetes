...
func (c *csiAttacher) Attach(spec *volume.Spec, nodeName types.NodeName) (string, error) {
	...
	if err != nil {
		glog.Error(log("attacher.Attach failed to get CSI source from spec: %v", err))
		return "", err
	}
	...
	if skip {
		glog.V(4).Infof(log("skipping attach for driver %s", driverName))
		return "", nil
	}
...
func (c *csiAttacher) VolumesAreAttached(specs []*volume.Spec, nodeName types.NodeName) (map[*volume.Spec]bool, error) {
	...
	for _, spec := range specs {
		...
		if err != nil {
			glog.Error(log("Failed to check CSIDriver for %s: %s", driver, err))
		} else {
			if skip {
...
func (c *csiAttacher) UnmountDevice(deviceMountPath string) error {
	...
	if err == nil {
		...
		// When this path is followed, it will not work for inline csi volumes
